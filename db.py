...
async def get_broker_orders(status: str | None = None, limit: int = 50) -> list[dict]:
...
async def update_broker_position(
    ...
    if row is None:
        # INSERT OR IGNORE so that a stale closed row with the same key (which
        # can exist on legacy schemas) does not raise UNIQUE constraint failure
        # and crash the executor after the broker order has already been placed.
        cur = await db.execute(
            ...
            (
                ...
                stop_px, take_px, initial_atr, atr_mult,
            ),
        )
        if cur.lastrowid == 0:
    ...
    if new_qty == 0:
        ...
        await db.execute(
            """UPDATE broker_positions
               ...
               WHERE id = ?""",
            (new_qty, new_lots, entry_px, stop_px, take_px, initial_atr, atr_mult, row[0]),
        )
    await db.commit()
